...
def construct_vector_store(embeddings, enable_qdrant=False):
    # 定义集合名称和向量维度
    ...
    if enable_qdrant:
        ...
        if os.path.exists(cache_path):
            try:
                ...
                print(f"加载本地向量库缓存失败({cache_path})，将重新构建：{e_load}")
        ...
        # 使用 InMemoryVectorStore 构建向量库，批处理以避免DashScope API限制
        vector_store = InMemoryVectorStore(embedding=embeddings)
        
        # DashScope API 限制每批最多10个文档
        batch_size = 10
        total_batches = (len(all_splits) + batch_size - 1) // batch_size
        
        print(f"开始批量添加文档到向量库，共 {len(all_splits)} 个文档，分 {total_batches} 批处理...")
        
        for i in range(0, len(all_splits), batch_size):
            batch = all_splits[i:i + batch_size]
            batch_num = i // batch_size + 1
            print(f"正在处理第 {batch_num}/{total_batches} 批，文档 {i+1} 到 {min(i+len(batch), len(all_splits))}...")
            
            try:
                vector_store.add_documents(batch)
                print(f"第 {batch_num} 批处理成功")
            except Exception as e:
                ...
                for j, doc in enumerate(batch):
                    try:
                        ...
                        print(f"  文档 {i+j+1} 添加成功")
                    except Exception as e_single:
                        print(f"  文档 {i+j+1} 添加失败: {e_single}")
        
        print("向量库加载完成（InMemoryVectorStore）。")
...
def generate(state: State):
    ...
    """
    system_prompt = """你是一位专业的糖尿病营养师。请根据用户提供的患者信息和相关知识，为该患者制定一份个性化的早餐建议。
